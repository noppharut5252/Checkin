...
import { LayoutGrid, Database, Loader2, Activity, ShieldCheck, FileText, MessageSquareWarning } from 'lucide-react';
...
import SystemFeedbackTab from './admin/SystemFeedbackTab';
...
const AdminCheckInManager: React.FC<AdminProps> = ({ data, user, onDataUpdate }) => {
    ...
    const [activeTab, setActiveTab] = useState<'locations' | 'activities' | 'printables' | 'logs' | 'live' | 'passport' | 'docs' | 'feedback'>('locations');
    ...
    return (
        <div className="pb-20 space-y-6 animate-in fade-in">
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col md:flex-row justify-between items-center gap-4">
                ...
                <div className="flex gap-2">
                    ...
                    <div className="flex flex-wrap bg-gray-100 p-1 rounded-lg">
                        <button onClick={() => setActiveTab('locations')} className={`px-3 py-2 rounded-md text-xs md:text-sm font-bold ${activeTab === 'locations' ? 'bg-white shadow text-blue-600' : 'text-gray-500'}`}>สถานที่</button>
                        <button onClick={() => setActiveTab('activities')} className={`px-3 py-2 rounded-md text-xs md:text-sm font-bold ${activeTab === 'activities' ? 'bg-white shadow text-blue-600' : 'text-gray-500'}`}>กิจกรรม</button>
                        <button onClick={() => setActiveTab('passport')} className={`px-3 py-2 rounded-md text-xs md:text-sm font-bold flex items-center ${activeTab === 'passport' ? 'bg-indigo-600 text-white shadow' : 'text-gray-500'}`}><ShieldCheck className="w-3 h-3 mr-1"/> Passport</button>
                        ...
                        <button onClick={() => setActiveTab('feedback')} className={`px-3 py-2 rounded-md text-xs md:text-sm font-bold flex items-center ${activeTab === 'feedback' ? 'bg-amber-500 text-white shadow' : 'text-gray-500'}`}><MessageSquareWarning className="w-3 h-3 mr-1"/> Feedback</button>
            ...
            {activeTab === 'feedback' && <SystemFeedbackTab data={data} user={user} />}
