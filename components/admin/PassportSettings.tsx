...
const PassportSettings: React.FC<PassportSettingsProps> = ({ data, onDataUpdate }) => {
    ...
    // Optimized Scan Handler: TRUST CLIENT & FAST LOCAL CHECK
    const handleScanResult = async (code: string) => {
        ...
        const now = Date.now();
        if (now - qrTime > 15 * 60 * 1000) { // Increased to 15 mins to be safe
            playSound('error');
        ...
        // 5. Check Duplicate Redemption (Local - Critical)
        ...
        // 6. BYPASS COMPLETION CHECK
        // We trust that the QR Code was generated by the User App ONLY after requirements were met.
        // This removes the dependency on syncing Logs to Admin device.
    ...
    const confirmRedemptionFromScan = async () => {
        ...
        redeemReward(user.UserID, mission.id).then(res => {
            if (res.status !== 'success') {
                ...
                // Keep local state anyway to prevent double scan attempts during network issue
